...
2. すでに `sshc` が走っている状態で、アプリをもう一度開く（束の実体、`sshc open`、あるいは `make desktop-run` をもう一度）。Electron の単一インスタンスロックにより**新しいウィンドウもエンジンも増えない**ことを確認する（`ps -ax | grep 'sshc'` が 1 つのまま）。ウィンドウが見当たらなければ、メニューバーの項目から開く。
...
9. アプリを 2 つ同時に起動し（束の実体を 2 回、あるいは `sshc open` を 2 つの端末から同時に）、`ps` に見えるエンジンが 1 つだけであることを確認する。
