...
7. 接続先を VPN の中の名前で書き、その名前を引ける DNS サーバーを指定して繋ぐ。`sshc vpn` の「target address」が期待したアドレスであること、ホストの `/etc/resolv.conf` が変わらないこと、同じ名前をホストで引くと別のアドレス（または引けない）であっても VPN の中のアドレスへ繋がることを確認する。名前を引く経路は、実際の VPN の DNS に対してまだ確認できていない。
