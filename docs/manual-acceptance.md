...
10. **アイコンが出ることを確認する。** 開発中（`make desktop-run`）は dock だけが sshc の図になり、**メニューバーの一番左は Electron のまま**である — あれは走っている束の Info.plist から来るので、JS からは変えられない。`make desktop-bundle-mac` で作った束を開くと、dock もメニューバーも sshc になる。
...
13. **窓を出さずに上がること（`--hidden`）を確認する。** 自動検査を置いていないので、ここで一度見る。`make desktop-bundle-mac` で作った束を一度開いて LaunchServices に覚えさせ、アプリを終了してから `open -g -b com.github.aida0710.sshc --args --hidden` を打つ。**メニューバーに項目が出て、窓は出ず、前面も奪わない**ことを確認する。項目から「ウィンドウを開く」でそこから窓が開くことも確認する。同じことが `sshc <接続先>` からも起きる — アプリが走っていない状態で打つと、この経路でアプリが上がってから接続する。
