...
14. desktopで2ペインへ切り替え、現在のtabが右側へ複製されることを確認する。左右で複数tabを追加・切替し、各tabのhost/pathが独立すること、1ペインへ戻して再び2ペインを開いても右tabが残ること、再読み込み後も両側のtabが復元されることを確認する。異なるhostを左右に開き、fileとdirectoryを片方からもう片方へDrag & Dropする。copyではsourceが残り、moveではtargetの完了後だけsourceが消えること、local端末に平文の一時fileが作られないことも確認する。2ペインを開いたままmobile幅へ縮めると、左tab列と1ペインだけになり、右ペイン、比較、2ペイン切替が動作しないことを確認する。desktop幅へ戻したときは右tabが復元されることを確認する。
