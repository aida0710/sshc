...
4. 単一terminalへ戻すと、Workspace名・台数・一括送信・保存レイアウト管理が表示されず、terminal名と検索toolbarは残ることを確認する。検索ボタンまたは`Ctrl/Cmd+F`で検索欄が開き、0台では保存済みレイアウトを開く入口が残る。
...
1. 検証用Serial deviceを接続し、`sshc serial`と`sshc serial --json`の双方へ同じdeviceが現れることを確認する。Linuxでは利用者がdeviceを開く権限も確認する。
...
7. Duo など二段階の装置で、OpenConnect のプロファイルに「電話の承認を待つ」を指定して繋ぐ。パスワードだけで通知が出る装置では「二段目に送る語」を空にし、もう一問聞いてくる装置では `push` を入れる。承認を待つあいだ、VPN 画面と `sshc vpn` に「電話の承認を待っています」と出ること、承認すると経路が立つこと、承認しなければ 2 分で理由付きで止まることを確認する。
8. 接続先を VPN の中の名前で書き、その名前を引ける DNS サーバーを指定して繋ぐ。`sshc vpn` の「target address」が期待したアドレスであること、ホストの `/etc/resolv.conf` が変わらないこと、同じ名前をホストで引くと別のアドレス（または引けない）であっても VPN の中のアドレスへ繋がることを確認する。名前を引く経路は、実際の VPN の DNS に対してまだ確認できていない。
9. WireGuard のプロファイルで相手の公開鍵をわざと違う値にし、`sshc vpn up` が「握手できなかった」理由で止まり、経路が開かないことを確認する。
10. OpenConnect の経路を開いたまま `sshc vpn down` し、数秒で終わること、装置の管理画面（または `occtl show users`）にセッションが残らないことを確認する。実際の AnyConnect 系装置に対しては未確認。
11. `sshc <alias>` で VPN 経由の接続を開いたまま 10 分以上待ち、経路が畳まれないことを確認する。接続を閉じてから 10 分後に畳まれることも確認する。
