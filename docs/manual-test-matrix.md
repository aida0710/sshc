...
>
> 束の smoke を消したあと、**出荷の道には実行検査が一本も無かった。** 後継は
> `scripts/ci/cli-smoke.{sh,ps1}` で、リリースの各 OS ジョブが自分の作った
> バイナリに対して走らせる。下記「いまの出荷検査」を参照。
> 記録の中に出てくる `scripts/{macos,linux,windows}/package-smoke.*` は、
> **束と一緒に消えている。**

## いまの出荷検査

リリースの各 OS ジョブは、自分がアップロードするそのバイナリを起こす。
...
| engine が居ないとき、次の一手を言う | 綴りではなく行動を返すこと |
| `sshc engine` が handoff を出す | 受け口・状態ディレクトリを実際に踏む |
| `sshc status` が走っている engine を報告する | 実体どうしの往復 |
| 入口が画面を返す | `go:embed` の中身が空でもビルドは通る |

**走らせられるのは host のアーキだけである。** 別アーキの成果物は名前しか
確かめられない。同じジョブが両方を上げるので、対応表の「実機で動く」は
依然として host 側にしか付かない。
