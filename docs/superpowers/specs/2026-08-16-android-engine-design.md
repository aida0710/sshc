...
## 4 つの未知数 — 全部倒れなかった（2026-08-16、実機で確認）

実装の順序ではなく、**方式が成立するかどうか**を決めるものだった。どれも設計へ
戻る事態にはならなかった。

1. **gomobile bind が Go 1.26.6 で動くか。** 動いた。`golang.org/x/mobile` を
   `go.mod` の tool として固定し、`gobind` を `go install` すれば AAR が出る。
2. **cgo リゾルバで名前が引けるか。** 引けた。実機から SSH セッションが繋がった。
3. **`creack/pty` が Android の `/dev/ptmx` で動くか。** 動いた。
4. **WebView から `ws://127.0.0.1:PORT` が CSP の下で通るか。** 通った。
   埋め込みターミナルがそのまま繋がる。

代わりに倒れたのは、想定していなかったものばかりだった——空のワークスペースで
`groups` が null になること、入口の URL を読み直していたこと、edge-to-edge の
挿入量が decor に消費されていたこと、キーバーがラベルの文字列を送っていたこと。
**方式ではなく、その周りが全部だった。**
