...
already connected terminal from the navigation onto the left, right, top, or
...
- A **console** owns a local or SSH process and its terminal stream.
...
2. Every visible terminal exposes four edge zones derived from pointer position.
...
6. Removing a pane dissolves a two-pane Live Workspace or removes only that pane
   from a larger one. It does not close any terminal connection.
...
- Local and SSH consoles may join the same workspace. Layout behavior depends on
  terminal capabilities rather than transport kind.
