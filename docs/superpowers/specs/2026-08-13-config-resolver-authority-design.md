...
| `Port` | `22` | 接続、画面、到達性 |
| `IdentityFile` | **持たない**（累積するが既定は無い） | 接続、鍵の利用状況 |
...
**`IdentityFile` の既定を持たないのは実測の結果である。** 一度は OpenSSH の既定の
並びを写したが、差分テストが macOS と Linux で違う答えを返した——Linux 側のビルドは
`~/.ssh/id_xmss` を含んでいた。版とビルドオプションで変わる表であり、この節の判断が
そこにも当てはまる。書かれていなければ答えない。接続に使う鍵を選ぶのは B2 であり、
そちらは OpenSSH の探索順ではなく、利用者が選んだ鍵と鍵の一覧を使う。
...
`RemoteForward`、`DynamicForward`、`SendEnv` は、最初の 1 つが勝つのではなく積み上がる。

**`SetEnv` は積み上がらない。実機の `ssh -G` で確かめた。** 二行書くと最初の行しか
出力されない——複数の変数は `SetEnv ONE=1 TWO=2` と一行に並べる。`internal/application`
から引き継いだ表がここを誤っていた。
...
**D. 既定値。** `Port 22`、`User`＝ローカルの user 名、`HostName`＝alias。決定事項 1 の
表の範囲に限る。`IdentityFile` の既定は、実測の結果として持たない。
...
- トークン展開（`%h` `%p` `%r` `%u` `%d` `%n`）
- 累積するキーワード（`IdentityFile` を複数）と、積み上がらないもの（`SetEnv` を複数行）
- 既定値（`Port`、`User`）
...
- **`Match final` の二周目。** OpenSSH は `final` を含む設定を二度読む。その二周目を
  この解決器は持たないので、**B1 では `Refusal` として拒む**。実装したふりをするより、
  答えられないと言う方が正しい。差分テストで確かめられるようになってから足す
