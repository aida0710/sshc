...
- カード外周は子要素に隠れるinset shadowではなく、`--ui-control-line`の実borderで描きます。通常のカード、control、menu、dialog、選択行の角は4pxへ統一し、接続状態badgeなど意味のあるpillだけ`rounded-full`を使用します。分割paneの左右headerは同じ最小高さとpaddingに揃えます。
