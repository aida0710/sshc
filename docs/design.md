...
- `make integration`は固定digestのOpenSSHコンテナに対してSFTP subsystemを実際に開き、upload／file・directory download／chmod／text save／競合拒否／rename／list／deleteを往復します。環境変数がない通常の`go test ./...`ではskipし、利用者のホストへ接続しません。
