...
- **ブラウザ登録**は、engine を再起動しても bookmark や PWA から入り直せるようにするための、端末固有の資格情報です。bootstrap を通ったブラウザに登録 token を発行し、`localStorage` に置かせます。engine は `~/.ssh/sshc/browser-registrations.json` に token のハッシュと発行時刻・最終使用時刻だけを保存し、`POST /api/v1/session/recover` は `X-SSHC-Browser` の token を検証して cookie と CSRF を発行します（bootstrap と同じく `Origin` の完全一致を要求し、CSRF は要りません）。この token は plain HTTP の固定 loopback origin に置く再利用可能な bearer なので、次の 4 つで被害を区切ります。(1) recover のたびに token を差し替え、応答の `browserToken` をブラウザが保存する。差し替え前の token は同じブラウザの別 tab のために 1 分だけ最新の token を返し、それを過ぎて提示されたら複製されたものとみなして登録ごと失効する。(2) 最後の使用から 30 日で登録を消す。(3) 登録は port（origin）に束縛し、既定 port の fallback でも `--port`／設定による固定でも、port が変わればすべて失効する。(4) 登録は最大 16 件で、古いものから消す。ブラウザのセッション（cookie）は engine の再起動で消えるほか、12 時間使われないか、発行から 7 日で切れます。切れたセッションは登録から黙って入り直すので、利用者には見えません。Menu の「このブラウザーからサインアウト」は `POST /api/v1/session/sign-out` でセッションを消し、`X-SSHC-Browser` を添えた場合は登録も消します（`localStorage` の token はブラウザ側が捨てます）。登録が消えた時点で、その登録から発行された猶予期間内の旧 token もすべて失効します。連続した rotation で旧 token の猶予期間を延長しません。想定しない脅威は、同一端末の別 OS ユーザーが engine 停止中に同じ port を占有して bookmark からの訪問を待ち伏せることです。この場合も奪える token は次の recover まで有効な 1 つで、正規のブラウザが再び recover した時点で複製側が失効します。
...
- 通常の設定文書は保存・読み込みとも 1 MiB を上限とします。背景画像は専用の容量設定に従い、絶対上限は 1 GiB です。背景画像の改名・削除・同期・世代 backup・復元にも同じ画像上限を適用し、暗号化 backup の読み込みでは envelope による増加分を確保します。
...
- Host の識別は「正規化した相対パス + 具体的な主 alias」です。alias と設定ファイルの改名は config と metadata を同一トランザクションで更新し、対応先が消えた metadata は推測で付け替えず orphan として再関連付けを求めます。グループはディレクトリなので、グループを変えるとパスが変わり、識別も変わります。移動は config と metadata を同一トランザクションで更新するため、識別が変わっても orphan にはなりません。Vault 解錠中の alias 改名では、パスワード・TOTP の割り当てと startup snippet の binding も同じ atomic transaction に含め、失敗時は config を含めて巻き戻します。移動先に残った専用パスワードを、移動元の名前付きパスワードの認証先 binding で解放しません。
...
- ターミナルの行数・列数は、表示枠とxtermの描画面の両方を監視して再計算します。DPIの変更やWebGLからの復帰では、表示枠が同じでも文字の寸法が変わるためです。再計測は次の描画フレームへまとめ、描画面の変更と再計測が同じResizeObserver通知内で循環するのを避けます。算出したサイズはPTYにも送り、最下行まで表示枠内に収めます。

## スマホの表示と操作

- `useMediaQuery.ts`の`mobileViewportQuery`を共通の切り替え条件とします。幅767px以下、または主ポインターがcoarseで幅1023px以下の場合にスマホ用ナビゲーション、単一ペイン、ターミナル補助キーを使います。横向きのスマホでも、幅だけでdesktop用2ペインへ切り替えません。SFTPの一覧は、desktopではペインが狭くなっても列を減らさずテーブルを横スクロールし、2行表示の一覧はスマホ（`mobileViewportQuery`）だけに使います。コンテナー幅（680px未満）は、ツールバーのボタンを操作メニューへ畳む判定と右ペインの非表示にだけ使います。
...
- パスフレーズを保存するのは、利用者がVaultを作成した場合だけです。ssh-agent への委譲は従来どおり利用者の明示的な操作で、既定では何も保存しません。保存する場合の境界は次節にまとめます。

## アプリケーションの施錠と秘密の保管庫の境界

- 初回起動ではVaultを作成し、パスワードあり（4文字以上・文字種制限なし）またはパスワードなしを選びます。作成・解錠するまで UI を利用できません。`/api/v1` は `vault_locked` で拒否します。例外は解錠前に必要な vault 自身の route、`GET /api/v1/health`、`POST /api/v1/session/bootstrap`、`POST /api/v1/session/renew` です。この制御は middleware に集約し、`Unlocked` が未設定の server は常に施錠状態とします。
- パスワードありの保管庫は実行環境に関係なく、既定では最後の使用から 12 時間で自動施錠します。Settingsで1〜999分／時間、または自動施錠なしを選択でき、保存直後と次回起動時の両方で同じ設定を適用します。自動施錠なしでも、明示的な施錠とengine終了時には導出済みの鍵を破棄します。状態取得は使用として扱いません。タブを開いたままにするだけでタイムアウトが延長されるのを防ぐためです。
- この認証は `~/.ssh` 自体を暗号化しません。config と鍵は OpenSSH が読み取れる形式で保存されます。パスワードありのVaultは、ファイルだけを持ち出された場合にもパスワードの強度に応じた保護を提供します。同じOSユーザーとして実行中のアプリを制御される場合は隔離できません。

- 保管庫は `~/.ssh/sshc/secrets` の 1 ファイルです。AES-256-GCM で暗号化し、パスワードありでは鍵をマスターパスワードから Argon2id で導出します。マスターパスワードは保存も送信もしません。復旧経路もないため、忘れた場合は内容を復元できません。
- パスワードなしでは暗号学的乱数32 bytesを`~/.ssh/sshc/local-vault-key`へ保存し、そのhex表現から同じArgon2idで鍵を導出します。空パスワードでは暗号化しません。保存は既存storage層のprivate file権限を使い、OS資格情報ストアとの連携はありません。端末鍵と暗号文の両方を読める主体には保護を提供しません。端末鍵は同期の収集・受信から大文字小文字違いも含め除外します。
- パスワードなしではengine起動時に自動解錠し、idle timeoutは適用しません。CLI／ブラウザーの手動ロック要求は解錠状態を維持します。Webではロックボタンとコマンドパレットのロック項目を非表示にします。engine終了時にはメモリ上の鍵を破棄します。パスワード保護への切り替えでは端末鍵ファイルを空のmarkerへ置換し、Vault・同期設定・Snippet・既存backupを一つのatomic rekey transactionで再暗号化します。逆方向も同じtransactionを使い、旧世代の退避内容はcommit後に破棄します。起動時には中断した作成・rekey transactionを先に復旧し、commit前は旧世代へ戻し、commit済みなら後片付けを完了してから端末鍵を読みます。
- 同期キーの最低12文字という条件は維持し、ローカルVaultの最低4文字と分離します。APIの明示的な空文字はパスワードなしを表します。CLIのcreate/change-passwordでも新しいパスワードを空欄にすることで選択できます。change-passwordは現在のパスワード入力直後に認証済みCLI専用のverify routeで検証し、誤りなら新しいパスワードを尋ねません。パスワードレスではCLI・Webとも現在パスワード入力を省略します。Webは保護方式を取得してから変更を許可し、変更成功時には入力欄とロック項目も更新します。事前検証ではVaultの解錠状態を変えず、変更確定時にも現在パスワードを再検証します。
...
- `~/.ssh/sshc/backups/` の世代バックアップは、設定ファイル、秘密鍵、vault のいずれも現在のVault鍵で暗号化します。`storage.Manager` には暗号化関数だけを渡し、storage 層は秘密の形式を扱いません。読み取りは `ReadBackup` に集約し、rollback と履歴からの復元もこの経路を使用します。
...
- そのため **rollback と履歴からの復元には解錠済みのVaultが必要です**。アプリケーション自体がその後ろにあるので、実際には常に開いています。
...
- 前回同期したファイルがローカルから消えていれば、ローカル側の削除として比較します。リモートが未変更なら削除を維持し、リモートも編集されていれば競合にします。「ローカルを採用」で消したファイルを復活させず、「リモートを採用」でのみリモート側の内容へ戻します。
...
- engine の継続実行は OS の process supervisor に委ねます。`sshc engine` は foreground で動作するため、`tmux`、`systemd` user unit、`launchd` などを使用できます。具体例は [docs/headless-examples.md](headless-examples.md) を参照してください。`sshc service install`はLinuxでは`~/.config/systemd/user/sshc.service`、macOSでは`~/Library/LaunchAgents/io.github.aida0710.sshc.plist`を作成し、`status`と`disable`を含めてsshc管理marker付き定義だけを管理します。同名の手書き定義は上書きも削除もしません。変更操作は対象と実行ファイルを表示して確認を求め、user単位のOS file lockで直列化し、削除前に内容を再検証します。Windows service、scheduled task、desktop login itemは作成しません。

  常駐対象は `sshc engine` のforeground processです。引数なしの`sshc`はengineを起動しないため、サービス定義の実行入口には使用しません。登録するbinaryは管理元を確認できたHomebrewのformula安定パス、またはreceipt対応`install.sh`の配置先に限ります。install／restartの成功はsystemdまたはlaunchdのPID、handoff PID、status APIで確認します。`sshc update`は管理定義の全内容が更新前に照合した実行パスから生成される内容と一致し、activeのときだけ再起動し、停止中の定義、手書き定義、別導入の定義には触れません。
...
- WebSocket endpoint は `/api/` の外にある `/terminal/stream` です。ブラウザは WebSocket handshake にカスタム header を付けられないため、`X-SSHC-CSRF` を必須とする `/api/` 配下には配置できません。代わりに、CSRF 検証済みの `POST /api/v1/terminal/sessions` が単回 ticket を発行します。ticket は 1 個の session ID と、そのブラウザーが描画済みの絶対byte cursorに紐付き、10 秒で失効し、1 回だけ使用できます。初回は保持中の全出力、再接続はcursor以後の差分だけを、`start`／`next`／`end`／`truncated` metadataに続けて送るため、既存のbrowser scrollbackを消去せず重複もしません。切断中の出力がengine ringから脱落していた場合はdecoderを初期化し、欠落を画面で通知します。無効、期限切れ、使用済みの場合は upgrade せず 403 を返します。upgrade 時には `Origin` の完全一致も確認します。
...
- 転送キュー `sshc/transfers.json` は保存・再読込とも 32 MiB を上限とします。上限を超えた書き込みは既存の保存済みキューを維持して失敗します。旧版が保存した上限超過キューは同じディレクトリの `.sshc-transfers.json.corrupt-*` へ移し、engine は空のキューで起動します。退避ファイルは同期対象外です。
...
- 同じ engine 内の Monaco Editor 保存と upload 公開は、SSH alias と正規化済みtarget pathの組ごとに直列化します。これにより両操作が同じrevisionを同時に検証して互いを上書きすることはありません。ただし一般的な SFTP v3 には「revision が一致するときだけ rename」を行うatomic CASがないため、別のSSH clientやremote processが検証とrenameの間に書き換える競合の検出はbest-effortです。新規fileの公開（upload、put、copy）は標準renameを使い、検証後に同名が現れた場合はOpenSSHのsftp-serverが上書きを拒否します。posix-renameによる置換は利用者が上書きを選んだ場合だけです。rename・remove・chmod と remote 間 move の削除も同じbest-effortの境界にあり、move は copy 直後の metadata revision と一致した file だけを削除します。
- server が返す entry 名は信用しません。空、`.`、`..`、`/` や NUL を含む名前を返す listing は、一覧・copy・delete・archive のすべてで `invalid_path` として失敗させ、黙って飛ばしません。engine の local file system へ書く download と archive は `\` も拒否します。revision は接頭辞で種類を区別し、`content-sha256:` は editor・download・upload のどれで得ても同じ内容なら一致します。
...
終了確認はキャンセル側に初期フォーカスと可視の輪郭を置き、モーダル外の遅延focusを戻す。ナビゲーションのセッション選択表示はTerminal画面でのみ出す。セッション行の主ボタンは余白も受け取り、メニューと終了のボタンだけを独立したクリック対象として上に置く。折りたたんだSFTP転送マネージャーはヘッダー全体で展開でき、操作メニューは独立して扱う。
...
貼り付けの既定割り当てはCtrl+V / Meta+V / Ctrl+Shift+V。いずれもブラウザーのpaste eventで処理し、Clipboard APIの追加readを起こさない。
