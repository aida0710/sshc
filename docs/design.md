...
- Workspace Command Centerは別契約です。preview evidenceへ展開後commandのdigest、各terminal session ID、SSH alias、表示名、process generationを含め、単回の`terminal.command.broadcast` tokenで確認します。送信時は同じconnected SSH sessionとgenerationだけに`command + "\r"`を全量入力し、再接続後の新しいshell、connecting／reconnecting／exited session、local shellを拒否します。通常打鍵と送信frameはsession単位で直列化します。PTY出力とexit codeをcommand単位には分離せず、APIは`delivered`／`failed`だけを返し、出力は各terminalのstreamとscrollbackへ流します。secret変数付きSnippetはTTY echoとshell履歴に残り得るため拒否します。
