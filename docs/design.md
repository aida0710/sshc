...
- 保管庫は実行環境に関係なく、既定では最後の使用から 12 時間で自動施錠します。Settingsで1〜999分／時間、または自動施錠なしを選択でき、保存直後と次回起動時の両方で同じ設定を適用します。自動施錠なしでも、明示的な施錠とengine終了時には導出済みの鍵を破棄します。状態取得は使用として扱いません。タブを開いたままにするだけでタイムアウトが延長されるのを防ぐためです。
