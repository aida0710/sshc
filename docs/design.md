...
- パスワードなしではengine起動時に自動解錠し、idle timeoutは適用しません。CLIのvault lockは解錠状態を維持します。ブラウザーからの手動施錠とengine終了時にはメモリ上の鍵を破棄し、パスワードなしで再解錠できます。パスワード保護への切り替えでは端末鍵ファイルを空のmarkerへ置換し、Vault・同期設定・Snippet・既存backupを一つのatomic rekey transactionで再暗号化します。逆方向も同じtransactionを使い、旧世代の退避内容はcommit後に破棄します。起動時には中断した作成・rekey transactionを先に復旧し、commit前は旧世代へ戻し、commit済みなら後片付けを完了してから端末鍵を読みます。
- 同期キーの最低12文字という条件は維持し、ローカルVaultの最低4文字と分離します。APIの明示的な空文字はパスワードなしを表します。CLIのcreate/change-passwordでも新しいパスワードを空欄にすることで選択できます。change-passwordは現在のパスワード入力直後に認証済みCLI専用のverify routeで検証し、誤りなら新しいパスワードを尋ねません。パスワードレスでは現在パスワード入力を省略します。事前検証ではVaultの解錠状態を変えず、変更確定時にも現在パスワードを再検証します。
