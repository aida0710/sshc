...
- カード外周は子要素に隠れるinset shadowではなく、低コントラストな`--ui-line`の実borderで描きます。大きなsurface、dialog、選択グループは12px、inputとbuttonは8px、小さな補助要素は6pxを基準にします。表の内部や連続したrowには角を付けず、接続状態badgeなど意味のあるpillだけ`rounded-full`を使用します。分割paneの左右headerは同じ最小高さとpaddingに揃えます。
