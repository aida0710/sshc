...
- パスワードなしではengine起動時に自動解錠し、idle timeoutは適用しません。手動施錠・終了ではメモリ上の鍵を破棄しますが、手動施錠後もパスワードなしで再解錠できます。パスワード保護への切り替えでは端末鍵ファイルを空のmarkerへ置換し、Vault・同期設定・Snippet・既存backupを一つのatomic rekey transactionで再暗号化します。逆方向も同じtransactionを使い、旧世代の退避内容はcommit後に破棄します。起動時には中断した作成・rekey transactionを先に復旧し、commit前は旧世代へ戻し、commit済みなら後片付けを完了してから端末鍵を読みます。
- 同期キーの最低12文字という条件は維持し、ローカルVaultの最低4文字と分離します。APIの明示的な空文字はパスワードなしを表します。CLIのcreate/change-passwordでも新しいパスワードを空欄にすることで選択できます。
