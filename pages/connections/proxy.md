...
sshcは`ProxyJump`を内部でたどり、踏み台から最終接続先まで、一貫した方法でホスト鍵と認証情報を確認します。
...
踏み台と最終接続先は、別々の認証先です。それぞれのエイリアスに、必要な鍵、鍵のパスフレーズ、アカウントパスワードを保存してください。2FAなどの追加入力は、対話Terminalで行います。
