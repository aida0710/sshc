...
sshcはSSHサーバーでも、クラウド中継サービスでもありません。接続と復号は手元の端末で行い、Terminalのスクロールバックや実行中のプロセスをクラウドへ保存しません。同期用ストレージも提供せず、同期データは端末上で暗号化してから、利用者が用意したS3互換ストレージへ保存します。OpenSSH設定は独自形式へ変換しないため、VS Code、Codex、通常の`ssh`コマンドからも同じ設定を利用できます。
