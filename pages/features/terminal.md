...
- WebGLによる描画。WebGLを利用できない場合、または背景画像を使用する場合は、透明セルを正しく再描画できるDOM描画へ自動で切り替え

再接続時の出力復元に使うエンジン側のバッファと、ブラウザーのスクロールバックは、どちらもメモリにだけ保持します。Vault、バックアップ、同期スナップショットには保存しません。
