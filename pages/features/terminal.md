...
［Settings］→［Terminal］の［接続のログ］では、新しく開く接続について、接続段階、試した認証方式、保存済みTOTPを利用できたか、経由地、所要時間を表示できます。［すべて（-vvv）］を選ぶと先頭に現在のログレベルも表示します。パスワード、TOTPコード、セットアップキーはログへ出しません。`sshc ssh <接続先> --non-interactive`も同じ設定を使用します。
