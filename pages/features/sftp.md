...
デスクトップでは`2ペイン`へ切り替え、2つのリモート接続先またはディレクトリを並べて開けます。切り替えた直後は、現在のタブが右ペインにも複製されます。左右に独立したタブ列と`+`があり、それぞれ最大8タブまで別の接続先やディレクトリを開けます。タブがペインの幅を超えるとタブ列だけを横へスクロールでき、`+`は右端に残ります。新しく開いたタブや選んだタブは、自動で見える位置へ移動します。各タブの接続先と開いていた場所は端末に保存され、`1ペイン`へ戻しても右側のタブは次に開くまで保持されます。

ファイルやフォルダーを、現在表示している一方のタブから反対側のタブへドラッグすると、コピーまたは移動を選べます。データは2つのSFTP接続間で直接ストリームされ、ローカル端末へ平文の一時ファイルを作りません。同じ接続先の移動では、可能な場合はサーバー上のrenameを使います。
...
狭い画面ではファイル名を主表示とし、権限、サイズ、更新日時をその下の1行へまとめます。スマートフォンでは横スクロールできるタブ列と1ペインだけを表示し、2ペイン用の接続や比較処理は動かしません。デスクトップへ戻ると、保存していた右側のタブを再び利用できます。
