...
`sshc vpn add`は新しいプロファイルを作成します。同じ名前のプロファイルがすでにある場合は何も変更せずにエラーになり、`sshc vpn remove`で削除してから追加するか、`sshc vpn rename`で名前を変更するよう案内します。設定はターミナルで対話的に入力します。接続先をホスト名で指定する場合は、その名前を解決するVPN内のDNSサーバーも入力します。OpenConnectでは、パスワードのあとに追加の認証を求めるサーバーのために、二要素認証の方法（`none`／`approve`／`totp`）も入力します。秘密鍵、VPNのパスワード、IPsecの事前共有鍵は入力時に表示せず、コマンド引数や環境変数からは受け取りません。保存先はVaultです。

`sshc vpn`の一覧には、経路が起動していれば、トンネルのインターフェース名、トンネルのアドレス、接続開始時刻も表示されます。

`sshc vpn remove`と`sshc vpn rename`にはVaultのロック解除が必要です。`sshc vpn rename`は、設定・秘密・接続への割り当てをまとめて新しい名前へ移します。接続中の経路はいったん切断しますが、新しい名前が使用できない場合は切断しません。

`sshc vpn logs`は、そのプロファイルのコンテナの直近のログを表示します。保存済みの秘密は`[REDACTED]`に置き換えます。接続できないときは、まずこのログを確認してください。

`sshc vpn proxy`は、標準入出力をその経路へ中継します。ホストの`ssh`・`scp`・`git`から経路を使うためのコマンドで、`~/.ssh/config`に次のように書きます。
...
`%h %p`を渡すと、その接続先へ届く経路かどうかを確認してから中継します。プロファイルの接続先と一致しない場合は、通常のネットワークへ切り替えずに接続を中止します。省略した場合は確認しません。SSHのハンドシェイクと鍵は`ssh`側で扱い、sshcはデータを中継するだけです。
...
対話ターミナルで`sshc sftp get`を実行すると、エンジンがリモートファイルを準備している間の進捗を接続ごとに表示します。分割している場合は接続ごとの進捗バーになります。分割しない転送は1本です。`--json`や出力先が対話ターミナルでない場合は進捗表示を行わず、機械処理する出力を汚しません。
