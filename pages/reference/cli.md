...
選択した接続先の通信だけを専用のVPN経由にします。Dockerが動作しているマシンで利用できます。詳しくは[接続ごとのVPN](/features/vpn)を参照してください。
...
`sshc vpn add`は新しいプロファイルを作成します。同じ名前のプロファイルがすでにある場合は何も変更せずにエラーになり、`sshc vpn remove`で削除してから追加するか、`sshc vpn rename`で名前を変更するよう案内します。設定はターミナルで対話的に入力します。接続先をホスト名で指定する場合は、その名前解決に使うVPN内のDNSサーバーも入力します。OpenConnectでは、パスワードのあとに追加の認証を求めるサーバーのために、二要素認証の方法（`none`／`approve`／`totp`）も入力します。秘密鍵、VPNのパスワード、IPsecの事前共有鍵は入力時に表示せず、コマンド引数や環境変数からは受け取りません。保存先はVaultです。

`sshc vpn`の一覧には、VPNが接続中であれば、トンネルのインターフェース名、トンネルのアドレス、接続開始時刻も表示されます。

`sshc vpn remove`と`sshc vpn rename`にはVaultのロック解除が必要です。`sshc vpn rename`は、設定、シークレット、接続先への割り当てをまとめて新しい名前へ移します。接続中の経路はいったん切断しますが、新しい名前が使用できない場合は切断しません。
...
`%h %p`を渡すと、そのVPN経由で接続先へ到達できるかを確認してから中継します。プロファイルの接続先と一致しない場合は、通常のネットワークへ切り替えずに接続を中止します。省略した場合は確認しません。SSHのハンドシェイクと鍵は`ssh`側で扱い、sshcはデータを中継するだけです。

`bind`で割り当てた接続は、Terminal、SFTP、`sshc <接続先>`のどれから開いても同じ経路を使います。経路を用意できないときは、通常のネットワークへ切り替えずに接続を中止します。
