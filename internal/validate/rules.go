...
// ここにあるのは、ブラウザにも同じ結果を出してほしい規則である。
//
// パターンは文字列として持つ。そのまま TypeScript の RegExp へ渡すからで、
// 表記が 2 つあれば、いつか片方だけが直る。使ってよいのは Go の RE2 と JavaScript の
...
const (
	...
	// その表記は Include のパスに現れる。alias で同じ理由から拒んでいるのと同じ
...
// その表記を作る操作だからである。`config` という名前の鍵を空のワークスペースに
...
// GroupName は、この表記を connections の下のディレクトリとして受け付けてよいかを
// 報告する。
...
// Reserved は、この表記が ~/.ssh の中で既に意味を持つかを報告する。
//
// 比較は大小文字を区別しない。既定の macOS ボリュームは "Config" と "config" を
// 同じディレクトリエントリとして扱う。
...
//
// 長さはバイトで数える。パターンの {0,63} は Go ではバイト、JavaScript では
// UTF-16 の単位を数えるので、そこだけは両者で意味が違う。文字集合が ASCII に
// 限られているぶん、受理される表記では一致する。受理されないものについては、
// ブラウザの方が緩い側へ外れるので、契約は保たれる。
