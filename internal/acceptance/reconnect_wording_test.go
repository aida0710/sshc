...
//
// 設定画面は「5 回（最大 33 秒）」と書いている。その 33 は
// `internal/terminal` の backoff（1・2・5・10・15 秒）の和である。
//
// **和が二か所にあると、片方だけが古くなる。** 間隔を伸ばした人は Go 側を
// 直して緑を見て、画面はその日から嘘をつき始める——そして読む人は、画面の方を
