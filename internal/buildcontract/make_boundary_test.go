...
func main() {
	...
	keys := []string{"GOENV", "GOOS", "GOARCH", "CGO_ENABLED", "SSHC_NATIVE_GOOS", "SSHC_NATIVE_GOARCH", "SSHC_NATIVE_CGO", "SSHC_NATIVE_OUTPUT", "SSHC_NATIVE_VERSION"}
...
`
	...
	for _, key := range []string{"GOOS", "GOARCH", "CGO_ENABLED"} {
		if value, present := capture.Environment[key]; present {
			t.Errorf("host go run inherited %s=%q", key, value)
		}
	}
}
