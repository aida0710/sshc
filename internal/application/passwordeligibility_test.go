...
func TestStoredPasswordTokenIsDisabledWhenItsEnvironmentCanReachAnotherProcess(t *testing.T) {
	for name, entry := range map[string]string{
		"proxy command": "Host target\n\tProxyCommand helper %h %p\n",
		"proxy jump":    "Host target\n\tProxyJump bastion\n",
		"match":         "Host target\n\tHostName target.example\nMatch all\n\tUser ops\n",
		"send env":      "Host target\n\tSendEnv SSHC_*\n",
		"send key path": "Host target\n\tSendEnv SSHC_ASKPASS_KEY_PATH\n",
		"key provider":  "Host target\n\tPKCS11Provider /tmp/provider.so\n",
	} {
		t.Run(name, func(t *testing.T) {
			...
			allowed, err := service.StoredPasswordAllowed("target")
			if err != nil {
				t.Fatal(err)
			}
			if allowed {
				t.Fatal("a child process could inherit the account-password token")
			}
		})
	}
}
