...
//
// **2 で、保管庫は封ではなく中身として運ばれるようになった。** 古いビルドがこれを
// 読めば `sshc/secrets.json` を知らないファイルとして扱い、平文のままディスクへ
// 書く——保存されたパスワードの写しがそこに残る。版を上げてあるので、そうはならず
// 「新しいビルドが作ったスナップショット」として断られる。
//
// 読む側は 1 も受け取る。あちらでは保管庫は封のまま入っており、それは移行の前に
// 押し出されたスナップショットそのものである。
...
type Entry struct {
	...
	// パスワードで封じられるようになった時点でその理由は消え、pull は鍵も他の
