...
export function WorkspaceCommandCenter({
  ...
  paneTargets: WorkspaceCommandTarget[];
  onClose: () => void;
}) {
  ...
  const [dispatch, setDispatch] = useState<TerminalCommandDispatch | null>(
    null,
  );
  ...
  const selectedSnippet = useMemo(
    () => snippets.find((item) => item.id === snippetId) ?? null,
    [snippetId, snippets],
  );
  const targets = useMemo(
    () =>
      paneTargets.filter(
        (target): target is WorkspaceCommandTarget & { sessionId: string } =>
          target.connected && target.sessionId !== undefined,
      ),
    [paneTargets],
  );
  const targetFingerprint = paneTargets
    .map(
      (target) =>
  ...
  function request(): TerminalCommandRequest | null {
    const executionTargets = targets.map(({ targetId, sessionId }) => ({
      targetId,
      sessionId,
    }));
    ...
    if (source === "command")
      return command.trim() === ""
        ? null
        : { command, targets: executionTargets, inputs: {} };
    return snippetId === ""
      ? null
      : { snippetId, targets: executionTargets, inputs };
  }
  ...
  async function run() {
    ...
    setProblem("");
    try {
      const nextDispatch = await terminalCommandApi.dispatch(
        prepared.preview,
        prepared.request,
      );
      if (operationSequence.current === operation) {
  ...
  function targetLabel(target: {
    ...
    const suffix =
      pane === undefined
        ? ""
        : ` · ${t("workspace.paneNumber", { number: pane.paneNumber })}`;
    ...
    >
      <div className="flex flex-col gap-3">
        <div className="flex items-center gap-3">
          <div className="grow">
            <h2
              id="workspace-command-heading"
              className="text-base font-semibold"
            >
              {t("workspace.broadcastHeading")}
            </h2>
            <p className="mt-1 text-xs text-ink-muted">
              {t("workspace.broadcastDescription")}
            </p>
          </div>
          <button
            ref={closeButton}
        ...
        <div className="grid gap-3 lg:grid-cols-[minmax(18rem,0.9fr)_minmax(20rem,1.1fr)]">
          <div className="flex min-w-0 flex-col gap-3">
            <Segmented
              label={t("workspace.commandSource")}
              value={source}
              options={[
                { value: "command", label: t("workspace.adHocCommand") },
                { value: "snippet", label: t("workspace.savedSnippet") },
              ]}
              onChange={(value) => {
                setSource(value);
                setInputs({});
            ...
            {source === "command" ? (
              <label className="text-xs text-ink-muted">
                ...
                <textarea
                  ...
                  placeholder="uname -a"
                  className="mt-1 block w-full rounded border border-control-line bg-control p-2 font-mono text-sm text-ink"
                />
              </label>
            ) : (
              <>
                ...
                {selectedSnippet === null ? null : (
                  <code className="whitespace-pre-wrap rounded bg-code-bg p-2 text-xs text-code-fg">
          ...
          <div className="flex min-w-0 flex-col gap-3">
            <div className="flex flex-wrap items-center gap-3">
              <span className="text-xs font-medium">
                {t("workspace.targetMode")}
              </span>
              <span className="text-xs text-ink-muted">
                {t("workspace.targetCount", { count: targets.length })}
              </span>
            </div>
            <div className="grid max-h-36 grid-cols-1 gap-1 overflow-auto sm:grid-cols-2">
              {paneTargets.map((target) => (
                <div
                  key={target.targetId}
                  className={`rounded border border-line px-2 py-1 text-xs ${target.connected ? "" : "opacity-60"}`}
                >
                  <span className="font-medium">{target.alias}</span>
                  <span className="ml-2 text-ink-muted">
                    {target.title} ·{" "}
                    {t("workspace.paneNumber", { number: target.paneNumber })}
                  </span>
                  {target.connected ? null : (
                    <span className="ml-2 text-notice-ink">
                      {t("workspace.targetSkipped", { state: target.state })}
                    </span>
                  )}
                </div>
              ))}
        ...
        {prepared === null ? null : (
          <section className="rounded-lg border border-notice-line bg-notice p-3">
            <h3 className="text-sm font-medium">
              {t("workspace.previewHeading")}
            </h3>
            <div className="mt-2 grid max-h-48 gap-2 overflow-auto md:grid-cols-2">
              {prepared.preview.targets.map((target) => (
                <div key={target.targetId} className="min-w-0">
                  <p className="truncate text-xs font-medium">
                    {targetLabel(target)}
                  </p>
                  <pre className="mt-1 overflow-auto rounded bg-code-bg p-2 text-xs text-code-fg">
                    {target.command}
                  </pre>
                </div>
              ))}
            ...
            <Button
              ...
              onClick={() => void run()}
            >
              {t("workspace.sendTargets", {
                count: prepared.preview.targets.length,
              })}
            </Button>
          </section>
        )}
        {dispatch === null ? null : (
          <section className="rounded-lg border border-line bg-card p-3">
            <h3 className="text-sm font-medium">
              {t("workspace.deliveryResults")}
            </h3>
            <p className="mt-1 text-xs text-ink-muted">
              {t("workspace.deliveryNotice")}
            </p>
            <div className="mt-2 grid max-h-56 gap-2 overflow-auto md:grid-cols-2">
              {dispatch.results.map((result) => (
                <div
                  key={result.targetId}
                  className="min-w-0 rounded border border-line p-2"
                >
