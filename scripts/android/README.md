...
KVMを利用できるAPI 36 Emulatorを起動し、debug APKを作成してからrepository rootで実行します。Android SDKにはplatform-toolsとbuild-toolsの`aapt`が必要です。`aapt`が別の場所にある場合は`SSHC_ANDROID_AAPT`に実行ファイルのパスを指定できます。
