...
- **インストールは `make install`** です。`~/.local/bin/sshc` へ原子的に入れます（sudo 不要）。1 箇所に固定するのはこのアプリでは特に効きます。`SSH_ASKPASS` と Terminal 起動は実行時のバイナリの絶対パスをその場で埋め込むため、リポジトリを移動したり別チェックアウトでビルドすると保存済み鍵パスフレーズでの起動が静かに壊れます。ログイン時起動が既にオンなら、install はそれも同じバイナリへ付け替えて再起動します。オフなら勝手にオンにはしません。再起動後の vault は施錠され、開いていた Web セッションは切れるため、`sshc open` または通常の `sshc` から入り直してください。`make uninstall` は先にログインサービスを解除し、成功した場合だけバイナリを削除します。
