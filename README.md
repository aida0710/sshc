...
- Effective タブと Diagnostics タブは値の出所説明のみです。`ssh -G` による実効設定判定、到達性診断、埋め込みターミナル、鍵管理、Known Hosts は後続サブシステムで実装します。
...
- 接続テスト、`known_hosts` 変更、`ssh-keyscan`、公開鍵のリモート登録、危険な設定での `ssh -G` は、CSRF header に加えて、対象と操作種別と表示済みコマンドへ紐付いた一回限りの action token（`X-SSHC-Action`）を必要とします。digest はサーバ側で発行時と使用時の両方で計算するため、確認から実行までの間に設定を編集すると token は無効になります。
...
- 自動テストは実リモートと実 `~/.ssh` を使いません。例外は二つです。一時ディレクトリ内の安全な fixture に対する `ssh -G -F` の差分試験（`ssh` が無い環境では skip）と、`/bin/echo` を本物の PTY で起こす 1 本の検査です。後者はリモートにもユーザーの設定にも触れません。
