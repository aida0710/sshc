...
- 埋め込みターミナルは安全な文字集合の alias に限ります。それ以外は `unsafe_alias` として拒否し、コマンドのコピーだけを提供します。`GET /api/v1/terminal/command` は残っており、自分の端末へ貼るための文字列を返します。**起動可否はもう報告しません**——このアプリケーションは端末アプリケーションを起こさなくなったので、その問い自体が無くなりました。
