...
- **開いているコンソールの一覧は一番左のナビゲーションにあります。** `Start / Home` を上に固定し、その下を「設定」と「ターミナル」のトグルで丸ごと入れ替えます。「設定」側は従来どおりのセクション一覧です。既定はターミナル側ですが、セッションが 1 本も無ければ設定側から始めます — セッションはプロセスの寿命までしか生きないので、無条件にターミナル側を既定にすると毎回の起動が空の一覧から始まってしまいます。トグルの状態は覚えず、毎回この規則で決まります。
