...
- **ローカルシェルは ssh 接続ではありません。** `localhost` は Home の接続一覧に出しません。あの一覧は `~/.ssh/config` の投影であり、`localhost` はそこに存在しないからです。入口はサイドパネルの「ローカルシェル」だけです。シェルは `$SHELL`（絶対パスで、実在し、実行可能な場合のみ）、次に `/bin/zsh`・`/bin/bash`・`/bin/sh` の順に探します。`PATH` は見ません。argv[0] には先頭にハイフンを付けてログインシェルとして起こします。
